...
Output only the text, nothing else.""",
        ...
        # Debug: inspect raw Moondream output before cleanup.
        print(f"  [Moondream raw]: '{text_en[:100]}'")
...
def start_auto_translator():
    print("=========================================")
    print("Deltarune Auto Translator Ready!")
    print("Auto translate on dialog open (F9 fallback enabled)")
    print("Hold 'ESC' to exit")
    print("=========================================")
    ...
    with mss.mss() as sct:
        ...
        while True:
            ...
            # Ensure at least one dialog box is detected.
            if any(len(r.boxes) > 0 for r in results):
                # Hash only the detected dialog crop to ignore background changes.
                box = results[0].boxes[0]
                x1, y1, x2, y2 = map(int, box.xyxy[0])
                crop_for_hash = img[y1:y2, x1:x2]

                # Make hash robust against tiny color noise/flicker.
                low_res_gray = cv2.cvtColor(crop_for_hash, cv2.COLOR_BGR2GRAY)
                _, low_res_gray = cv2.threshold(low_res_gray, 50, 255, cv2.THRESH_BINARY)
                current_hash = hashlib.md5(low_res_gray.tobytes()).hexdigest()

                if current_hash != pending_hash:
                    pending_hash = current_hash
                    pending_time = time.time()
                    print("  [Auto] Typing detected...")
                else:
                    if pending_time is not None:
                        elapsed = time.time() - pending_time
                        if elapsed >= STABLE_WAIT and current_hash != last_stable_hash:
                            print(f"[Auto] Stable detected ({elapsed:.1f}s) -> Translating...")
                            process_screen_image(img, use_dedup=False)
                            last_stable_hash = current_hash

                if keyboard.is_pressed('f9'):
                    print("[Manual] F9 pressed -> translating")
                    process_screen_image(img, use_dedup=False)
            else:
                pending_hash = None
                pending_time = None
