...
def fuzzy_fix_ocr(raw_text: str) -> str:
    ...
    # ลบกลุ่มตัวอักษรสั้นๆ ที่เป็น noise เช่น J, E, JE
    text = re.sub(r'(?<![A-Za-z])\b[KkTtIlJjEe]{1,2}\b(?![A-Za-z])', '', text)
...
def ocr_best_text(crop, box_index=0):
    # ✅ เอา has_character_portrait ออกทั้งหมด ไม่ต้องตัดซ้ายแล้ว
    ...
    for j, line_crop in enumerate(lines):
        cv2.imwrite(f"debug_box{box_index}_line{j+1}.png", line_crop)
        
        best_line, best_conf = "", -1.0

        pad = 10
        line_padded = cv2.copyMakeBorder(
            line_crop, 0, 0, pad, 0,
            cv2.BORDER_CONSTANT, value=(0, 0, 0)
        )
        
        for variant in build_ocr_variants(line_padded):
            result = reader.readtext(variant, detail=1, paragraph=False, allowlist=OCR_ALLOWLIST)
            if not result: continue
            
            result = sorted(result, key=lambda r: r[0][0][0])
            texts = [r[1] for r in result if len(r) >= 3]
            confs = [r[2] for r in result if len(r) >= 3]
            candidate = " ".join(texts).strip()
            if not candidate: continue
            
            avg_conf = sum(confs) / len(confs)
            if avg_conf > best_conf:
                best_conf, best_line = avg_conf, candidate
                
        if best_line:
            all_texts.append(best_line)
            total_conf += best_conf
            count += 1
    
    full_text = " ".join(all_texts)
    avg = total_conf / count if count else 0.0
    return re.sub(r"\s+", " ", full_text).strip(), avg
